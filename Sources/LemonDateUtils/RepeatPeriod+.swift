...
public func calculateLastRepeatDateBeforeEndDate(
  startDate: Date, endDate: Date, repeatPeriod: RepeatPeriod, n: Int
) -> Date {
  ...
  case .weekly:
    ...
    result = max(
      result, calendar.date(byAdding: .day, value: numberOfPeriods * n * 7, to: startDate)!)

  case .monthly:
    let monthsBetween = calendar.dateComponents([.month], from: startDate, to: endDate).month!
    let numberOfPeriods = monthsBetween / n
    result = max(
      result, calendar.date(byAdding: .month, value: numberOfPeriods * n, to: startDate)!)
