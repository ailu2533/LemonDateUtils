...
public func calculateLastCustomWeeklyRepeatDateBeforeEndDate(
    startDate: Date, endDate: Date, customWeek: UInt8
) -> Date {
...
public func calculateLastRepeatDateBeforeEndDate(
    startDate: Date, endDate: Date, repeatPeriod: RepeatPeriod, interval: Int
) -> Date {
    ...
    case .daily:
        ...
        let numberOfPeriods = daysBetween / interval
        result = max(result, calendar.date(byAdding: .day, value: numberOfPeriods * interval, to: startDate)!)
    ...
    case .yearly:
        ...
        let numberOfPeriods = yearsBetween / interval
        result = max(result, calendar.date(byAdding: .year, value: numberOfPeriods * interval, to: startDate)!)
    }
